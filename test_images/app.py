...
if uploaded_file is not None and model is not None and len(class_names) > 0:
    ...
    if primary_confidence < CONFIDENCE_THRESHOLD:
        ...
        try:
            if hasattr(disease_info, 'info_dict') and primary_class in disease_info.info_dict:
                data_profile = disease_info.info_dict[primary_class]
                
                # Tabbed component wrapper creates a highly sleek, elite visual interface
                tab1, tab2, tab3 = st.tabs(["📋 Description & Pathology", "🧪 Biochemical Treatment", "🛡️ Agrosanitary Prevention"])
                
                with tab1:
                    st.markdown("### Profile Description")
                    st.write(data_profile.get('about', 'Data unavailable.'))
                    st.markdown("### Clinical Symptoms")
                    st.write(data_profile.get('symptoms', 'Data unavailable.'))
                    
                with tab2:
                    st.markdown("### Recommended Agricultural Controls")
                    st.write(data_profile.get('treatment', 'Data unavailable.'))
                    
                with tab3:
                    st.markdown("### Long-term Crop Field Management")
                    st.write(data_profile.get('prevention', 'Data unavailable.'))
            else:
                st.info("💡 Model identified pattern metrics successfully, but text-profile parameters for this class are missing inside `disease_info.py`.")
